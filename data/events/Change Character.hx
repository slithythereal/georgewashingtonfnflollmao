...
{
	if (charMap[strumIndex] == null)
		charMap[strumIndex] = [];
	if (charMap[strumIndex][charIndex] == null)
		charMap[strumIndex][charIndex] = new StringMap();
}
...
public function changeCharacter(strumIndex:Int, charName:String = 'bf', memberIndex:Int = 0, ?updateBar:Bool = true)
{
	...
	// null check
	if (oldChar.curCharacter == newCharacter.curCharacter)
		return trace('It\'s the same character bro.');
	if (oldChar == null || newCharacter == null)
		return;
	...
	if (memberIndex == 0 && updateBar)
	{
		...
		{ // opponent side
			...
			if (Options.colorHealthBar)
				healthBar.createColoredEmptyBar(newCharacter != null
					&& newCharacter.iconColor != null ? newCharacter.iconColor : (PlayState.opponentMode ? 0xFF66FF33 : 0xFFFF0000));
		}
		else if (strumIndex == 1)
		{ // player side
			iconP1.setIcon(newCharacter != null ? newCharacter.getIcon() : 'face');
			if (Options.colorHealthBar)
				healthBar.createColoredFilledBar(newCharacter != null
					&& newCharacter.iconColor != null ? newCharacter.iconColor : (PlayState.opponentMode ? 0xFFFF0000 : 0xFF66FF33));
		}
	}
	...
	trace('Character index "'
		+ memberIndex
		+ '" changed to "'
		+ newCharacter.curCharacter
		+ '" on strumLine "'
		+ strumIndex
		+ '"!');
}
